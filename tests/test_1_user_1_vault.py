...
def test_multi_deposit_addSettRewards(deployer, users, want_whale, badger_whale, vaults, badger_tree, badger, want):
    ...
    vault.deposit(toDeposit, {"from": users[0]})
    ...
    # same deposit as user 1 but at a later time
    vault.deposit(want.balanceOf(users[1]), {"from": users[1]})
    ...
    vault.deposit(toDeposit * 0.5, {"from": users[2]})
