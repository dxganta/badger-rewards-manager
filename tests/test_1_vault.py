...
def test_multi_deposit_addSettRewards(deployer, users, want_whale, badger_whale, vaults, badger_tree, badger, want):
    ...
    dai.transfer(badger_tree, dai_amount_1 + 5, {"from": deployer})
    crv.transfer(badger_tree, crv_amount_1 + 5, {"from": deployer})
    link.transfer(badger_tree, link_amount_1 + 5, {"from": deployer})
    badger_tree.addSettRewards(
